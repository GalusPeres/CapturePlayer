...
export default function RendererControls() {
  ...
  return <div className="flex items-center gap-3">
    <InfoHint info="Standard uses the browser video player. WebGL processes color and FSR on the GPU. Native uses Media Foundation on Windows, AVFoundation on macOS, or V4L2 on Linux. HDR requires a compatible capture format, driver and display. Changing the capture backend restarts capture.">
...
export function HdrControls() {
  ...
  useEffect(() => {
    if (!portable || !settings.nativeRenderer) return;
    let live = true;
    const inspect = async () => {
      try {
  ...
  const unavailable = !settings.nativeRenderer || (portable && (support?.key !== key || !support.possible));
  const checked = settings.nativeRenderer && settings.nativeHdr;
  ...
  return <div className="flex items-center gap-3">
    ...
    <InfoHint info={<>
      ...
      {portable && <span className="block mt-1">{support?.key === key ? support.reason : 'Checking capture formats…'}</span>}
    </>}>
